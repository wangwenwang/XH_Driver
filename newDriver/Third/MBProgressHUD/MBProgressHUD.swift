...
class MBProgressHUD: UIView {
    ...
    func show(_ animated: Bool) {
        ...
        if graceTime > 0.0 {
//            let newGraceTimer: Timer = Timer(timeInterval: graceTime, target: self, selector: "handleGraceTimer:", userInfo: nil, repeats: false)
...
//                minShowTimer = Timer(timeInterval: minShowTime - interv, target: self, selector: "handleMinShowTimer:", userInfo: nil, repeats: false)
...
//        }
    ...
    override func draw(_ rect: CGRect) {
        ...
        var centerX : CGFloat = boxRect.maxX - CGFloat(radius)
        var centerY : CGFloat = boxRect.minY + CGFloat(radius)
        var centerP : CGPoint = CGPoint.init(x: centerX, y: centerY)
        context.addArc(center: centerP, radius: CGFloat(radius), startAngle: 3 * CGFloat(M_PI) / 2, endAngle: 0, clockwise: false)
        
        centerX = boxRect.maxX - CGFloat(radius)
        centerY = boxRect.maxY - CGFloat(radius)
        centerP = CGPoint.init(x: centerX, y: centerY)
        context.addArc(center: centerP, radius: CGFloat(radius), startAngle:0, endAngle: CGFloat(M_PI) / 2, clockwise: false)
        
        centerX = boxRect.minX + CGFloat(radius)
        centerY = boxRect.maxY - CGFloat(radius)
        centerP = CGPoint.init(x: centerX, y: centerY)
        context.addArc(center: centerP, radius: CGFloat(radius), startAngle:CGFloat(M_PI) / 2, endAngle: CGFloat(M_PI), clockwise: false)
        
        centerX = boxRect.minX + CGFloat(radius)
        centerY = boxRect.minY + CGFloat(radius)
        centerP = CGPoint.init(x: centerX, y: centerY)
        context.addArc(center: centerP, radius: CGFloat(radius), startAngle:CGFloat(M_PI), endAngle: 3 * CGFloat(M_PI) / 2, clockwise: false)
...
class MBRoundProgressView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        if annular {
            ...
            let centerP : CGPoint = CGPoint.init(x: center.x, y: center.y)
            context.addArc(center: centerP, radius: radius, startAngle:startAngle, endAngle: endAngle, clockwise: false)
            context.closePath()
            context.fillPath()
        }
    }
...
//        CGContextAddArcToPoint(c: CGContext?, _ x1: CGFloat, _ y1: CGFloat, _ x2: CGFloat, _ y2: CGFloat, _ radius: CGFloat)
        
        var end1 : CGPoint = CGPoint.init(x: 2, y: 2)
        var end2 : CGPoint = CGPoint.init(x: radius + 2, y: 2)
        
        context.addArc(tangent1End: end1, tangent2End: end2, radius: radius)
        context.addLine(to: CGPoint(x: rect.size.width - radius - 2, y: 2))
        
        end1 = CGPoint.init(x: rect.size.width - 2, y: 2)
        end2 = CGPoint.init(x: rect.size.width - 2, y: rect.size.height / 2)
        context.addArc(tangent1End: end1, tangent2End: end2, radius: radius)
        
        end1 = CGPoint.init(x: rect.size.width - 2, y: rect.size.height - 2)
        end2 = CGPoint.init(x: rect.size.width - radius - 2, y: rect.size.height - 2)
        context.addArc(tangent1End: end1, tangent2End: end2, radius: radius)
        context.addLine(to: CGPoint(x: radius + 2, y: rect.size.height - 2))
        
        end1 = CGPoint.init(x: 2, y: rect.size.height - 2)
        end2 = CGPoint.init(x: 2, y: rect.size.height / 2)
        context.addArc(tangent1End: end1, tangent2End: end2, radius: radius)
        context.fillPath()
        ...
        end1 = CGPoint.init(x: 2, y: 2)
        end2 = CGPoint.init(x: radius + 2, y: 2)
        context.addArc(tangent1End: end1, tangent2End: end2, radius: radius)
        context.addLine(to: CGPoint(x: rect.size.width - radius - 2, y: 2))
        ...
        if amount >= radius + 4 && amount <= (rect.size.width - radius - 4) {
            ...
            end1 = CGPoint.init(x: 4, y: 4)
            end2 = CGPoint.init(x: radius + 4, y: 4)
            context.addArc(tangent1End: end1, tangent2End: end2, radius: radius)
            ...
            end1 = CGPoint.init(x: 4, y: rect.size.height - 4)
            end2 = CGPoint.init(x: radius + 4, y: rect.size.height - 4)
            context.addArc(tangent1End: end1, tangent2End: end2, radius: radius)
            ...
            var centerP : CGPoint = CGPoint.init(x: rect.size.width - radius - 4, y: rect.size.height / 2)
            context.addArc(center: centerP, radius: radius, startAngle:CGFloat(M_PI), endAngle: angle, clockwise: false)
            ...
            centerP = CGPoint.init(x: rect.size.width - radius - 4, y: rect.size.height / 2)
            context.addArc(center: centerP, radius: radius, startAngle:CGFloat(-M_PI), endAngle: angle, clockwise: true)
            context.addLine(to: CGPoint(x: amount, y: rect.size.height / 2))
